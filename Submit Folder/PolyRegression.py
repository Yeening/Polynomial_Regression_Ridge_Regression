...
def increase_poly_order(x, degree):
    ...
    for i in range(1,degree+1):
        result = np.concatenate((result,np.power(x,i)),axis=1)
    # your code
    return result
...
def get_loss_per_poly_order(x, y, degrees):
    ...
    x_sets = np.split(x,[int(x.shape[0]*0.6),int(x.shape[0]*0.8)],axis=0)
    y_sets = np.split(y,[int(y.shape[0]*0.6),int(y.shape[0]*0.8)],axis=0)
    x_train = x_sets[0]; x_val = x_sets[1]; x_test = x_sets[2];
    y_train = y_sets[0]; y_val = y_sets[1]; y_test = y_sets[2];
    for degree in degrees:
        x_poly_train = increase_poly_order(x_train,degree)
        x_poly_val = increase_poly_order(x_val,degree)
        theta = normal_equation(x_poly_train, y_train)
#         theta = np.zeros((x_poly_train[0].shape[0],1),dtype=float)
#         for i in range(num_iterations):
#             theta = theta + learning_rate * x_poly_train.T.dot(y_train - x_poly_train.dot(theta))
        y_train_predict = predict(x_poly_train, theta)
        train_loss = get_loss(y_train_predict,y_train)
        y_val_predict = predict(x_poly_val, theta)
        val_loss = get_loss(y_val_predict,y_val)
...
if __name__ == "__main__":
    ...
    plt.show()
#     plt.plot(x_train.T.tolist()[0], y_train.T.tolist()[0])
#     plt.plot(x_test.T.tolist()[0], y_test.T.tolist()[0])
#     plt.show()
    ...
    best_fit_plot(x, y, best_theta, best_degree)
...
#     print(best_degree)
    ...
    x_train_p = increase_poly_order(x_train, 4) 
    x_test_p = increase_poly_order(x_test, 4)
    gbest_thetas = gradient_descent(x_train_p, y_train, 0.005, 1000) 

#     best_fit_plot(x_train, y_train, gbest_thetas[-1], best_degree)
    plot_epoch_losses(x_train_p, x_test_p, y_train, y_test, gbest_thetas, "best learned theta with Degree4 - train, test losses vs. GD epoch ")
